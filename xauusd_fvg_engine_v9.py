"""
XAUUSD FVG Strategy - Engine v9.0
===================================
Wall Street Grade Backtest & Signal Engine

STRATEJİ AKIŞI (Şemaya Birebir - Haftalık Bias YOK):
──────────────────────────────────────────────────────
...
RİSK: 1:2 RR | %0.5 risk/işlem | Swing SL | Sadece London+NY seansı
"""
...
class MarketBrain:
    ...
    Haftalık bias YOK. Trend gate YOK.
...
class RiskManager:
    ...
    def compute(self, idx: int, direction: str,
        ...
        if direction == 'bull':
            swing = IndicatorEngine.swing_low(L, idx, lookback=30, strength=2)
            sl    = swing * (1.0 - self.sl_buffer)
            risk  = abs(entry - sl)
            tp    = entry + risk * self.rr
        else:
            swing = IndicatorEngine.swing_high(H, idx, lookback=30, strength=2)
            sl    = swing * (1.0 + self.sl_buffer)
            risk  = abs(sl - entry)
            tp    = entry - risk * self.rr
...
class BacktestEngine:
    ...
    def run(self, df_1h: pd.DataFrame, df_5m: pd.DataFrame,
        ...
        dbg = dict(session=0, no_signal=0, risk_fail=0, generated=0)
        ...
        for idx in bt_idx:
            ...
            if signal is None:
                hour = to_naive(TM[idx]).hour
                if not self.brain.in_session(hour):
                    dbg['session'] += 1
                else:
                    dbg['no_signal'] += 1
                continue
            ...
            # Risk hesapla
            entry = float(O[idx + 1])
            r = self.risk.compute(idx, signal.direction, entry, H, L, equity)
            if r is None:
                dbg['risk_fail'] += 1
                continue
...
class ReportGenerator:
    ...
    @staticmethod
    def save_csv(trades: List[Trade], filename: str = 'xauusd_fvg_v9_trades.csv'):
        if not trades:
            return
        rows = []
        for t in trades:
...
def main():
    ...
    if metrics:
        ...
        ReportGenerator.generate(trades, metrics, INITIAL_CAPITAL,
                                 out='xauusd_fvg_v9_report.png')
        ReportGenerator.save_csv(trades, 'xauusd_fvg_v9_trades.csv')
